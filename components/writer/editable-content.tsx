...
export function EditableContent({ messages, onChange, isLoading }: EditableContentProps) {
  ...
  return (
    <div className="relative flex flex-col h-[calc(100vh-17rem)]">
      {!isLoading && (
      ...
      {isEditing && !isLoading ? (
        <Textarea
          ...
          className="flex-1 font-mono text-sm p-4 resize-none h-full whitespace-pre-wrap break-words"
        />
      ) : (
        <div className="article-preview flex-1 border rounded-md p-4 bg-white overflow-auto">
          {messages.length === 0 ? (
