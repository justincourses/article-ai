...
export function SummaryImage({ summaryText, autoGenerate = false }: SummaryImageProps) {
  ...
  const [activeTab, setActiveTab] = useState<string>("unsplash"); // Default to Unsplash for all users

  // Set default tab based on VIP status
  useEffect(() => {
    // 只在组件首次渲染时设置默认标签，并且只有当 isVip 的值已确定时
    if (typeof isVip !== 'undefined') {
      const initialTab = isVip ? "ai" : "unsplash";
      setActiveTab(initialTab);
    }
    // 依赖 isVip，但只在其值首次确定时执行
  }, [isVip]);

  // 只有当 autoGenerate 为 true 时才自动生成图片
  useEffect(() => {
    // 使用 ref 来确保只执行一次
    if (initialRenderRef.current && summaryText && autoGenerate && isVip && activeTab === "ai") {
      initialRenderRef.current = false;
      generateImage(summaryText);
      setHasAttemptedGeneration(true);
    }
  }, [summaryText, autoGenerate, generateImage, isVip, activeTab]);
    ...
    <div className="mt-6 space-y-4">
      ...
      <Tabs value={activeTab} onValueChange={setActiveTab} className="w-full">
        <TabsList className="grid w-full grid-cols-2">
          <TabsTrigger value="unsplash">在线图库</TabsTrigger>
          <TabsTrigger value="ai" disabled={!isVip}>
            AI生成{!isVip && " (即将推出)"}
          </TabsTrigger>
        </TabsList>
        ...
        <TabsContent value="ai" className="mt-4">
          {isVip ? (
            ...
            <div className="p-4 text-sm text-amber-700 bg-amber-50 rounded-md">
              <p className="font-medium mb-1">VIP专属功能</p>
              <p>AI图片生成是VIP专属功能，请升级到VIP会员以使用此功能。</p>
            </div>
          )}
        </TabsContent>
      </Tabs>
